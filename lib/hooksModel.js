...
module.exports = {
	...
	afterCreate: function(item, options) {
		...
		return Promise.try(function() {
			...
			return through.findAll(queryOptions);
		})
	...
	beforeUpdate: function(item, options) {
		...
		.then(function(parentLevel) {
			...
			return Promise.try(function() {
				...
				return sequelize.query(sql, {transaction: options.transaction, replacements: {id: item.id, levelChange: levelChange}});
			})
			.then(function() {
				...
				return sequelize.query(sql, {transaction: options.transaction, replacements: {id: item.id}});
			})
			.then(function() {
				...
				return sequelize.query(sql, {transaction: options.transaction, replacements: {id: item.id, parentId: parentId}});
			});
		})
		.return();
	},
